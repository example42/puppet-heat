...
# = Define: heat::conf
#
# With this define you can manage any heat configuration file
#
# == Parameters
#
# [*template*]
...
#   Example: template => 'site/heat/my.conf.erb',
#
# [*content*]
...
#   Example: source => 'puppet:///site/heat/my.conf',
#
# [*ensure*]
...
define heat::conf (
  ...
  $manage_path    = pickx($path, "${heat::config_dir_path}/${name}")
  $manage_content = default_content($content, $template)
  $manage_mode    = pickx($mode, $heat::config_file_mode)
  $manage_owner   = pickx($owner, $heat::config_file_owner)
  $manage_group   = pickx($group, $heat::config_file_group)
  $manage_require = pickx($config_file_require, $heat::config_file_require)
  $manage_notify  = $config_file_notify ? {
    'class_default' => $heat::manage_config_file_notify,
    default         => $config_file_notify,
  }

  file { "heat_conf_${name}":
    ensure  => $ensure,
    source  => $source,
